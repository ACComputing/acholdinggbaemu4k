...
Fixed: removes fallback scanner interference, correct VRAM tracking,
accurate VBlank IRQ, and proper LZ77 handling.
"""
import sys
import time
import pygame
...
class Memory:
    def __init__(self):
        ...
        self.vram_written = False   # Track if game has written to VRAM
        self.pal_written = False    # Track palette writes
    ...
    def _read_io(self, off):
        if off == 0x000: return 0
        if off == 0x004: return 0
        if off == 0x006: return self.vcount
        if off == 0x200: return self.ie & 0x3FFF
        if off == 0x202: return self.if_ & 0x3FFF
        if off == 0x208: return self.ime
        if 0x100 <= off < 0x110: return self.timer[(off-0x100)//4] & 0xFF
        if 0xBA <= off < 0xE0: return (self.dma_cnt[(off-0xBA)//12] >> 16) & 0xFF
        return self.io[off]

    def _write_io(self, off, val):
        self.io[off] = val
        if off == 0x200: self.ie = val & 0x3FFF
        elif off == 0x202: self.if_ &= ~val
        elif off == 0x208: self.ime = val & 1
        elif 0x100 <= off < 0x110:
            idx = (off-0x100)//4
            if off & 2:
                self.timer_reload[idx] = (self.timer_reload[idx] & 0xFF) | (val << 8)
            else:
                self.timer_reload[idx] = (self.timer_reload[idx] & 0xFF00) | val
        elif 0x102 <= off < 0x110 and (off & 2):
            idx = (off-0x100)//4
            self.timer_control[idx] = val >> 8
            self.timer[idx] = self.timer_reload[idx]
        elif 0xBA <= off < 0xE0:
            idx = (off-0xBA)//12
            if (off & 0xF) == 0xA:
    ...
    def _trigger_dma(self, idx):
        ...
        if not (cnt & 0x02000000):
            self.dma_cnt[idx] &= ~0x80000000
    ...
    def update_timers(self, cycles):
        for i in range(4):
            ctrl = self.timer_control[i]
            if not (ctrl & 0x80):
                continue
            freq = [1, 64, 256, 1024][ctrl & 3]
            self.timer_prescale[i] += cycles
            while self.timer_prescale[i] >= freq:
                self.timer_prescale[i] -= freq
                self.timer[i] = (self.timer[i] + 1) & 0xFFFF
                if self.timer[i] == 0:
                    self.timer[i] = self.timer_reload[i]
                    if ctrl & 0x40:
                        self.request_interrupt(1 << (3 + i))
...
class ARM7TDMI:
    ...
    def _handle_irq(self):
        self.r_irq[0] = self.r[13]
        self.r_irq[1] = self.r[14]
        self.spsr_irq = self.cpsr
        self.cpsr = (self.cpsr & ~0x1F) | 0x12
        self.cpsr |= 0x80
    ...
    def _arm_swi(self, comment):
        ...
        if num == 0x00:  # SoftReset
            ...
            flags = self.r[0]
            if flags & 0x01: self.mem.ewram = bytearray(EWRAM_SIZE)
            if flags & 0x02: self.mem.iwram = bytearray(IWRAM_SIZE)
            if flags & 0x04: self.mem.pal = bytearray(PAL_SIZE)
            if flags & 0x08: self.mem.vram = bytearray(VRAM_SIZE)
            if flags & 0x10: self.mem.oam = bytearray(OAM_SIZE)
            if flags & 0x80: self.mem.io = bytearray(IO_SIZE)
            return 2
        elif num == 0x02:  # Halt
            self.halt = True
            return 2
            ...
            val = self.r[0]
            sqrt = int(val ** 0.5) if val < 0x80000000 else 0
            self.r[0] = sqrt
            return 2
            ...
            if word:
                for _ in range(length):
                    v = self.mem.read32(src)
                    self.mem.write32(dst, v)
                    if not fill: src += 4
                    dst += 4
            else:
                for _ in range(length):
                    v = self.mem.read16(src)
                    self.mem.write16(dst, v)
                    if not fill: src += 2
                    dst += 2
            return 4
            ...
            for _ in range(length):
                ...
                dst += 4
            return 4
        elif num in (0x11, 0x12):  # LZ77UnCompWRAM / VRAM
            src = self.r[0]; dst = self.r[1]
                    ...
                    if flags & 0x80:
                        ...
                        for _ in range(length):
                            ...
                            if vram:
                                ...
                                written += 1
                                if buf16_pos == 2:
                                    self.mem.write16(dst + written - 2, buf16)
                                    buf16 = 0
                                    buf16_pos = 0
                            else:
                        ...
                        if vram:
                            ...
                            written += 1
                    flags <<= 1
            if vram and buf16_pos == 1:
                self.mem.write8(dst + written - 1, buf16 & 0xFF)
            return 8
        return 2
    ...
    def _arm_dp(self, op):
        ...
        if opc == 0xD:
            ...
            res = self.r[rn] - op2
            self.update_nz(res)
            borrow = (self.r[rn] & 0xFFFFFFFF) < (op2 & 0xFFFFFFFF)
            if borrow: self.cpsr &= ~0x20000000
            else: self.cpsr |= 0x20000000
            a = self.r[rn]; b = op2
            v = ((a ^ b) & (a ^ res) & 0x80000000) != 0
            if v: self.cpsr |= 0x10000000
            else: self.cpsr &= ~0x10000000
            return 1
    ...
    def _exec_thumb(self, op):
        ...
        if cat == 0x00:
            ...
            rs = (op >> 3) & 7
            rd = op & 7
            self.r[rd] = (self.r[rs] << off) & 0xFFFFFFFF if off else self.r[rs]
            self.update_nz(self.r[rd])
            return 1
        if cat in (0x01, 0x02, 0x03):
            rd = op & 7
            rs = (op >> 3) & 7
            imm = (op >> 6) & 7
            opc = (op >> 9) & 3
        ...
        if cat == 0x04:
            ...
            if opc == 0: self.r[rd] = imm
                ...
                res = self.r[rd] - imm
                self.update_nz(res)
                borrow = (self.r[rd] & 0xFFFFFFFF) < (imm & 0xFFFFFFFF)
                if borrow: self.cpsr &= ~0x20000000
                else: self.cpsr |= 0x20000000
                return 1
        ...
        if cat == 0x05:
            ...
            if opc == 0: self.r[rd] = (self.r[rd] + self.r[rs]) & 0xFFFFFFFF
                ...
                self.update_nz(res)
                borrow = (self.r[rd] & 0xFFFFFFFF) < (self.r[rs] & 0xFFFFFFFF)
                if borrow: self.cpsr &= ~0x20000000
                else: self.cpsr |= 0x20000000
                return 1
        ...
        if cat == 0x0B:
            ...
            l = (op >> 11) & 1
            addr = self.r[rb] + imm
            if l: self.r[rd] = self.mem.read16(addr)
            else: self.mem.write16(addr, self.r[rd])
            return 2
        if cat == 0x0C:
            rd = (op >> 8) & 7
            imm = (op & 0xFF) * 4
            l = (op >> 11) & 1
            addr = self.r[13] + imm
            if l: self.r[rd] = self.mem.read32(addr)
            else: self.mem.write32(addr, self.r[rd])
            return 2
        if cat == 0x0D:
            rd = (op >> 8) & 7
            sp = (op >> 11) & 1
            imm = (op & 0xFF) * 4
            if sp: self.r[rd] = self.r[13] + imm
            else: self.r[rd] = (self.r[15] & ~2) + imm
            return 1
        if cat == 0x0E:
            imm = (op & 0x7F) * 4
            if op & 0x80: self.r[13] -= imm
            else: self.r[13] += imm
            return 1
        if cat == 0x0F:
            reg_list = op & 0xFF
...
class PPU:
    ...
    def update(self, force_fallback=False):
        ...
        # Only use fallback scanner if VRAM and palette were never written
        # (useful for debugging, disabled during normal gameplay)
        if force_fallback or (not self.mem.vram_written and not self.mem.pal_written and len(self.mem.rom) > 0):
    ...
    def _render_rom_fallback(self):
        # This is a debugging tool; it will not run if the game writes to VRAM.
        pass  # Disabled to avoid interference. If you want it back, remove the condition above.
...
class Emulator:
    def __init__(self):
        ...
        pygame.display.set_caption("acgbaemu - Fixed")
        ...
        while self.running:
            ...
            if self.rom_loaded and not self.paused:
                ...
                while cycles_this_frame < CYCLES_PER_FRAME:
                    c = self.cpu.step()
                    cycles_this_frame += c
                    self.mem.cycle_counter += c
                    self.mem.update_timers(c)
                    scanline = (self.mem.cycle_counter // CYCLES_PER_SCANLINE) % SCANLINES_TOTAL
                    self.mem.vcount = scanline
                    if scanline == VBLANK_START and self.mem.cycle_counter % CYCLES_PER_SCANLINE < c:
